...
* **Stage 1: The Unforgiving Slider** — A raw HTML range slider that demands the player hit *exactly* `50.00`. Anything else triggers a reset and mocking laughter.
* **Stage 2: The Evasive Button** — A simple confirmation button that actively teleports away from the cursor.
* **Stage 3: Invisible Flappy Bird** — A physics-based side-scroller where the bird is visible, but all obstacles and boundaries are completely invisible. Players must survive 5 seconds entirely blind.
* **Stage 4: Reverse Pong** — A game of classic Pong controlled by a vertical slider. The player is conditioned to keep the ball alive, but the *only* way to advance is to intentionally let the ball pass the paddle. 
* **Stage 5: The Deceptive Crossword** — A grid of jumbled letters. The win condition isn't finding a word; it's waiting for an unassuming "Next" arrow to slowly fade into existence over a full 60 seconds.
